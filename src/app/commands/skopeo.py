...
@click.command(
    "skopeo", help="Generate skopeo copy command script.\n\nSupported environment variables:\n\nHTTPS_PROXY, DOCKER_REGISTRY_HOST"
)
...
@click.option(
    "--disable-skopeo-proxy", "is_disable_skopeo_proxy", help="Execute the skopeo copy command without using a proxy?", is_flag=True
)
...
def cli(
    ...
    r = requests.get(
        "https://raw.githubusercontent.com/designinlife/version-checker/main/data/all.json",
            ...
            if "tags" in v and v["name"].startswith("docker-"):
                if is_latest_only:
                    for v2 in v["latest_tags"]:
                        cmds.append(
                            f"{f'HTTPS_PROXY={http_proxy} ' if http_proxy is not None and not is_disable_skopeo_proxy else ''}skopeo copy "
                        ...
                        for v3 in v["suffix"]:
                            cmds.append(
                                f"{f'HTTPS_PROXY={http_proxy} ' if http_proxy is not None and not is_disable_skopeo_proxy else ''}"
                    ...
                    if not repo_name or repo_name == v["repo"]:
                        for v2 in v["tags"]:
                            if (
                                v2["name"].startswith("sha256")
                                or v2["name"].endswith("-ubi")
                                or v2["name"].endswith("-ent")
                                or "debug" in v2["name"].lower()
                                or "rc" in v2["name"].lower()
                                or "beta" in v2["name"].lower()
                                or "alpha" in v2["name"].lower()
                                or "arm" in v2["name"]
                                or "amd" in v2["name"]
                                or "windows" in v2["name"]
                                or "nightly" in v2["name"]
                                or "rootless" in v2["name"]
                                or "builder" in v2["name"]
                            ):
                                continue
