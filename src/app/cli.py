...
from app.core.config import AppSetting, Configuration
from app.core.utils import safe_strtobool

logger.remove()
logger.add(sys.stderr, format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", level="ERROR", backtrace=True, diagnose=False)
...
if is_debug:
    logger.add(
        sys.stdout, format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", filter=lambda record: record["level"].no < 40, level="DEBUG"
    )
    ...
    logger.add(
        sys.stdout, format="{time:YYYY-MM-DD HH:mm:ss} | {level} | {message}", filter=lambda record: record["level"].no < 40, level="INFO"
    )
